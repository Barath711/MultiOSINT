...
_app_qt.setApplicationName("MultiOSINT v12")
...
class _CircularSplash(_QW):
    ...
    def paintEvent(self, _e):
        ...
        p.drawText(vr, _Qt.AlignmentFlag.AlignCenter, "v 12.0")
        # Percent
...
from PyQt6.QtCore import (
    ...
    QObject, QSize, QPoint, QRect, QByteArray, QThread, QMimeData)
...
class OSINTModule:
    ...
    @staticmethod
    def is_url(ioc):    return bool(re.match(r"https?://", ioc, re.I))
    ...
    def get_ioc_type(ioc):
        ...
        if OSINTModule.is_url(ioc):   return "url"
        return "domain"
    ...
    def check_virustotal(self, ioc, ioc_type):
        ...
        try:
            if ioc_type == "domain":
                url = f"{api}/domains/{ioc}"
            elif ioc_type == "url":
                uid = base64.urlsafe_b64encode(ioc.encode()).decode().strip("=")
                url = f"{api}/urls/{uid}"
            ...
            if resp.status_code == 200:
                ...
                if ioc_type == "ip":
                    ...
                    cats = attr.get("categories", {})
                    if cats: r["categories"] = ", ".join(list(set(cats.values()))[:4])
                    tnames = attr.get("threat_names", [])
                    r["threat_label"] = tnames[0] if tnames else ""
                elif ioc_type == "hash":
    ...
    def check_otx(self, ioc, ioc_type):
        ...
        otx_type = {"ip":"IPv4","domain":"domain","hash":"file","url":"url"}.get(ioc_type,"domain")
        try:
            resp = requests.get(
    ...
    def check_breachvip(self, ioc, field_type="email"):
        ...
        return r
    # ── EmailRep.io (free — no API key, 10 req/day anon) ──────────────────
    def check_emailrep(self, email_addr):
        r = {"er_verdict":"","er_reputation":"","er_suspicious":False,
             "er_credentials_leaked":False,"er_data_breach":False,
             "er_profiles":[],"er_error":""}
        try:
            from urllib.parse import quote
            resp = requests.get(
                f"https://emailrep.io/{quote(email_addr)}",
                headers={"User-Agent":"OSINTMultiSearch-v12"}, timeout=15)
            if resp.status_code == 200:
                d       = resp.json()
                ...
                r["er_reputation"]         = rep
                r["er_suspicious"]         = susp
                r["er_credentials_leaked"] = details.get("credentials_leaked", False)
                r["er_data_breach"]        = details.get("data_breach", False)
                r["er_profiles"]           = details.get("profiles", [])
                if susp or r["er_credentials_leaked"] or r["er_data_breach"]:
                    r["er_verdict"] = f"⚠️  Suspicious — Reputation: {rep}"
                else:
                    r["er_verdict"] = f"🟢 Clean — Reputation: {rep}"
            elif resp.status_code == 400:
                r["er_error"] = "EmailRep: Invalid email address"
            else:
                r["er_error"] = f"EmailRep HTTP {resp.status_code}"
        except Exception as e:
            r["er_error"] = f"EmailRep error: {e}"
        return r

    # ── LeakCheck.io public API (free — no API key required) ───────────────
    def check_leakcheck(self, email_addr):
        r = {"lc_found":0,"lc_sources":[],"lc_verdict":"","lc_error":""}
        try:
            from urllib.parse import quote
            resp = requests.get(
                f"https://leakcheck.io/api/public?check={quote(email_addr)}",
                headers={"User-Agent":"OSINTMultiSearch-v12"}, timeout=15)
            if resp.status_code == 200:
                d = resp.json()
                if d.get("success"):
                    ...
                    r["lc_found"]   = found
                    r["lc_sources"] = sources[:20]
                    r["lc_verdict"] = (
                        f"🔴 EXPOSED — found in {found} breach source(s)"
                        if found > 0
                        else "🟢 Not found in LeakCheck database")
                else:
                    r["lc_error"] = d.get("error","LeakCheck: unknown error")
            elif resp.status_code == 429:
                r["lc_error"] = "LeakCheck: Rate limited"
            else:
                r["lc_error"] = f"LeakCheck HTTP {resp.status_code}"
        except Exception as e:
            r["lc_error"] = f"LeakCheck error: {e}"
        return r
        ...
        try:
            if ioc_type == "hash":
                ...
                resp = requests.post("https://urlhaus-api.abuse.ch/v1/url/",
                    headers=auth, data={"url":ioc}, timeout=10, verify=False)
            else:
            ...
            if resp.status_code == 200:
                ...
                elif ioc_type == "url" and status in ("is_available","is_delivering"):
                    threat = d.get("threat",""); tags = d.get("tags") or []
                    url_status = d.get("url_status","")
                    r["uh_urls"] = 1
                    r["uh_threat"] = (f"🔴 {threat}" if threat else "🔴 Found in URLhaus") + (f" [{url_status}]" if url_status else "")
                    r["uh_tags"]   = ", ".join(tags[:6])
    ...
    def check_urlscan_search(self, ioc, ioc_type):
        r = {"us_malicious":0,"us_benign":0,"us_score":0,"us_tags":"","us_verdict":"","us_error":""}
        try:
            from urllib.parse import quote as _q
            q = f'page.url:"{_q(ioc,safe="")}"' if ioc_type == "url" else f"domain:{ioc}"
            resp = requests.get("https://urlscan.io/api/v1/search/",
                params={"q": q, "size": 10}, timeout=15, verify=False,
                headers={"User-Agent": "MultiOSINT-v11"})
            if resp.status_code == 200:
                results = resp.json().get("results", [])
                if not results:
                    r["us_verdict"] = "🟢 No scans found in URLScan"; return r
                verdicts = [res.get("verdicts",{}).get("overall",{}) for res in results]
                scores = [v.get("score",0) for v in verdicts if v.get("score") is not None]
                mal    = sum(1 for v in verdicts if v.get("malicious"))
                benign = sum(1 for v in verdicts if not v.get("malicious"))
                tags   = list(set(t for v in verdicts for t in (v.get("tags") or [])))[:5]
                max_score = max(scores) if scores else 0
                r.update({"us_malicious":mal, "us_benign":benign, "us_score":max_score,
                          "us_tags":", ".join(tags) if tags else "",
                          "us_verdict":("🔴 MALICIOUS" if mal > 0 else
                                        "🟡 Suspicious" if max_score >= 50 else "🟢 Clean")})
            elif resp.status_code == 429: r["us_error"] = "URLScan: Rate limit."
            else: r["us_error"] = f"URLScan search error: HTTP {resp.status_code}"
        except Exception as e: r["us_error"] = f"URLScan error: {e}"
        return r
...
NAV_TOOLS = {
    ...
    "📧 Email & Breach": [
        ...
        {"name": "EmailRep.io",      "url": "https://emailrep.io/",                                            "desc": "Email reputation + breach/credential leak check (free)","tags": ["email","breach"], "skipSearch": _NAV_SKIP},
        {"name": "LeakCheck.io",     "url": "https://leakcheck.io/",                                           "desc": "Check leaked credentials (free public API)",          "tags": ["email","breach"], "skipSearch": _NAV_SKIP},
...
# ══════════════════════════════════════════════════════════════════
# BROWSER HISTORY  —  helpers, data loading, worker
# ══════════════════════════════════════════════════════════════════
_BH_WEBKIT_EPOCH  = dt.datetime(1601, 1, 1, tzinfo=dt.timezone.utc)
_BH_ERROR_LOG     = os.path.join(os.environ.get("TEMP", "."), "history_export_error.log")
_BH_PREVIEW_LIMIT = 5_000

_BH_DL_STATE = {0: "In Progress", 1: "Complete", 2: "Cancelled",
                3: "Interrupted", 4: "Interrupted"}

def _bh_webkit_ts_to_utc(ts_micro):
    if ts_micro is None: return ""
    try:
        val = int(ts_micro)
        if val <= 0: return ""
        utc_dt = _BH_WEBKIT_EPOCH + dt.timedelta(microseconds=val)
        return utc_dt.strftime("%Y-%m-%d  %H:%M:%S") + "  UTC"
    except Exception: return ""

def _bh_log_error(e):
    try:
        with open(_BH_ERROR_LOG, "a", encoding="utf-8") as f:
            f.write("\n=== ERROR ===\n")
            f.write("Time: " + dt.datetime.utcnow().isoformat(timespec="seconds") + " UTC\n")
            f.write(traceback.format_exc() + "\n")
    except Exception: pass

def _bh_safe_copy_db(src_path):
...
def _bh_table_exists(conn, table):
    try:
        cur = conn.cursor()
...
def _bh_get_table_columns(conn, table):
    if not _bh_table_exists(conn, table): return []
    try:
        cur = conn.cursor()
...
def _bh_first_nonempty(row, col_idx, *names):
    for n in names:
        if n in col_idx:
            v = row[col_idx[n]]
...
def _bh_base_filename(*paths):
    for p in paths:
        if p and str(p).strip():
            try: return os.path.basename(str(p).strip())
            except Exception: continue
    return ""

def _bh_load_data(db_path, use_host_match):
    result = {"history": [], "downloads": [], "error": None,
              "hist_count": 0, "dl_count": 0}
    tmp_path = None; conn = None
    try:
        tmp_path = _bh_safe_copy_db(db_path)
        try:    conn = sqlite3.connect(f"file:{tmp_path}?mode=ro", uri=True)
        except: conn = sqlite3.connect(tmp_path)

        useful_dl = ["id","tab_url","site_url","referrer","url",
                     "target_path","current_path","suggested_path","full_path",
                     "start_time","end_time","bytes_received","total_bytes","state","opened"]
        dl_cols   = _bh_get_table_columns(conn, "downloads")
        select_dl = [c for c in useful_dl if c in dl_cols]
        if "id" not in select_dl and dl_cols: select_dl.insert(0, "id")
        ...
        if select_dl:
            ...
            if _bh_table_exists(conn, "downloads_url_chains"):
                try:
                    chain_rows = cur.execute(
                        "SELECT id, chain_index, url FROM downloads_url_chains ORDER BY id, chain_index"
                    ).fetchall()
                    tmp_chains: Dict[int, List] = defaultdict(list)
                    ...
                    for did, lst in tmp_chains.items():
                        ...
                        chain_map[did] = urls
                        if urls: final_url_map[did] = urls[-1]
                except Exception as e: _bh_log_error(e)

            for row in raw_dl:
                try:
                    did       = int(row[ci["id"]]) if "id" in ci else 0
                    path_vals = [row[ci[c]] for c in
                                 ["target_path","current_path","suggested_path","full_path"] if c in ci]
                    file_name  = _bh_base_filename(*path_vals)
                    final_url  = final_url_map.get(did)
                    source_url = (final_url or _bh_first_nonempty(row, ci, "tab_url","site_url","referrer","url"))
                    url_chain  = " → ".join(chain_map.get(did, []))
                    start_utc  = _bh_webkit_ts_to_utc(row[ci["start_time"]] if "start_time" in ci else None)
                    end_utc    = _bh_webkit_ts_to_utc(row[ci["end_time"]]   if "end_time"   in ci else None)
                    raw_state  = row[ci["state"]] if "state" in ci else None
                    state_str  = _BH_DL_STATE.get(raw_state, str(raw_state) if raw_state is not None else "")
                    tb = row[ci["total_bytes"]] if "total_bytes" in ci else None
                    try: tb = int(tb) if tb is not None else 0
                    except: tb = 0
                    ...
                    drec = {"id": did, "file_name": file_name or "", "source_url": source_url or "",
                            "final_url": final_url or "", "url_chain": url_chain,
                            "start_utc": start_utc, "end_utc": end_utc, "state": state_str, "size": size_str}
                    downloads_rows.append(drec)
                    if source_url and file_name:
                        exact_url_to_files[source_url].add(file_name)
                        host = urlparse(source_url).netloc.lower()
                        if host: host_to_files[host].add(file_name)
                except Exception as e: _bh_log_error(e)
        ...
        if _bh_table_exists(conn, "urls"):
            ...
            for (url, title, last_visit_time) in raw_hist:
                utc_str = _bh_webkit_ts_to_utc(last_visit_time)
                files: Set[str] = set()
                if url:
                    s_url = str(url).strip()
                    ...
                    if use_host_match:
                        ...
                        if host: files |= host_to_files.get(host, set())
                files_str = "; ".join(sorted(files)) if files else ""
                history_rows.append({"utc_time": utc_str, "url": url or "",
                                     "title": title or "", "downloaded": bool(files_str),
                                     "downloaded_files": files_str})
        result["history"]    = history_rows
        result["hist_count"] = len(history_rows)

    except Exception as e:
        _bh_log_error(e); result["error"] = str(e)
    finally:
        if conn:
            try: conn.close()
            except: pass
        if tmp_path:
            try: os.remove(tmp_path); os.rmdir(os.path.dirname(tmp_path))
            except: pass
    return result

def _bh_export_xlsx(data, outdir, prefix):
    if not HAS_OPENPYXL: raise RuntimeError("openpyxl required.  pip install openpyxl")
    wb      = _Workbook()
    ws_hist = wb.active; ws_hist.title = "History"
    ...
    for r in data["history"]:
        ws_hist.append([r["utc_time"],r["url"],r["title"],"Yes" if r["downloaded"] else "No",r["downloaded_files"]])
    ws_dl.append(["id","file_name","source_url","final_url","url_chain","start_time_utc","end_time_utc","state","size"])
    for r in data["downloads"]:
        ws_dl.append([r.get("id",""),r.get("file_name",""),r.get("source_url",""),
                      r.get("final_url",""),r.get("url_chain",""),
                      r.get("start_utc",""),r.get("end_utc",""),r.get("state",""),r.get("size","")])
    for ws in [ws_hist, ws_dl]:
        for col_idx, col_cells in enumerate(ws.columns, start=1):
            max_len = max((len(str(c.value or "")) for c in col_cells), default=0)
            ws.column_dimensions[_get_col_letter(col_idx)].width = min(max_len + 2, 80)
...
class _BHLoadWorker(QThread):
    finished = pyqtSignal(dict)
    error    = pyqtSignal(str)
    def __init__(self, db_path, use_host_match):
        super().__init__()
        self.db_path        = db_path
        self.use_host_match = use_host_match
    def run(self):
        try:
            result = _bh_load_data(self.db_path, self.use_host_match)
            if result.get("error"): self.error.emit(result["error"])
...
QTableWidget{background:"""+bg+""";color:"""+fg+""";border:none;gridline-color:"""+bdr+""";alternate-background-color:"""+bg2+"""}
QTableWidget::item{padding:4px 8px;border:none}
QTableWidget::item:selected{background:"""+acc+""";color:#fff}
QHeaderView::section{background:"""+bg2+""";color:"""+fg3+""";border:none;border-bottom:1px solid """+bdr+""";padding:6px 8px;font-weight:600;font-size:12px}
QCheckBox{color:"""+fg+""";spacing:8px}
QCheckBox::indicator{width:18px;height:18px;border-radius:5px;border:2px solid """+bdr+""";background:"""+bg2+"""}
QCheckBox::indicator:checked{background:"""+acc+""";border-color:"""+acc+"""}
QTabWidget::pane{background:"""+bg+""";border:none;border-top:1px solid """+bdr+"""}
QTabBar{background:"""+bg2+""";border-bottom:1px solid """+bdr+"""}
QTabBar::tab{background:transparent;color:"""+fg3+""";border:none;padding:10px 18px;font-size:13px}
QTabBar::tab:selected{color:"""+acc+""";font-weight:600;border-bottom:2px solid """+acc+"""}
QTabBar::tab:hover{color:"""+fg+""";background:"""+bg3+"""}
"""
...
# ── Plain-text copy helper (fixes paste double-spacing issue) ────────
class _PlainCopyTextEdit(QTextEdit):
    """QTextEdit that copies as clean plain text – no HTML paragraph spacing."""
...
class ColoredTextEdit(_PlainCopyTextEdit):
    TAG_COLORS_DARK = {
...
# ══════════════════════════════════════════════════════════════════
# BROWSER HISTORY PANEL  (embedded version of BrowsingHistoryGUI)
# ══════════════════════════════════════════════════════════════════
class BrowserHistoryPanel(QWidget):
    """Full Browser History Analyzer UI embedded as a QWidget panel."""
    ...
    def __init__(self, dark=True, parent=None):
        ...
        self._worker:         Optional[_BHLoadWorker] = None
        self._build_ui()
    ...
    def _build_header(self) -> QFrame:
        ...
        lay = QHBoxLayout(hdr); lay.setContentsMargins(18, 12, 18, 12)
        logo = QLabel("🌐  Browser History Analyzer")
        logo.setFont(QFont("Segoe UI", 15, QFont.Weight.Bold))
        logo.setProperty("accent", True); lay.addWidget(logo)
        sub = QLabel("Edge / Chrome History Explorer  ·  All timestamps in UTC")
        sub.setProperty("dim", True); sub.setFont(QFont("Segoe UI", 10))
        lay.addWidget(sub); lay.addStretch()
        return hdr

    # ─── Sidebar ──────────────────────────────────────────────────
    def _build_sidebar(self) -> QFrame:
        side = QFrame(); side.setProperty("sidebar", True)
        side.setMinimumWidth(255); side.setMaximumWidth(340)
        lay = QVBoxLayout(side); lay.setContentsMargins(14, 16, 14, 14); lay.setSpacing(10)

        _lbl = QLabel("History File"); _lbl.setFont(QFont("Segoe UI", 11, QFont.Weight.Bold))
        lay.addWidget(_lbl)

        sel_btn = QPushButton("📂  Select History File(s)…")
        sel_btn.setProperty("accent", "true"); sel_btn.setMinimumHeight(38)
        sel_btn.clicked.connect(self._select_files); lay.addWidget(sel_btn)

        self._files_lbl = QLabel("No files selected")
        self._files_lbl.setProperty("dim", True); self._files_lbl.setFont(QFont("Segoe UI", 9))
        self._files_lbl.setWordWrap(True); lay.addWidget(self._files_lbl)
        self._bh_sep(lay)

        _lbl2 = QLabel("Output"); _lbl2.setFont(QFont("Segoe UI", 11, QFont.Weight.Bold))
        lay.addWidget(_lbl2)
        lbl_out = QLabel("Output folder:"); lbl_out.setFont(QFont("Segoe UI", 9))
        lbl_out.setProperty("dim", True); lay.addWidget(lbl_out)

        out_row = QHBoxLayout(); out_row.setSpacing(6)
        self._out_dir_edit = QLineEdit(self._output_dir)
        self._out_dir_edit.setPlaceholderText("Output folder…"); out_row.addWidget(self._out_dir_edit)
        brw = QPushButton("…"); brw.setFixedWidth(36)
        brw.clicked.connect(self._browse_outdir); out_row.addWidget(brw)
        lay.addLayout(out_row)

        lbl_sub = QLabel("Subject label (prefix, optional):")
        lbl_sub.setFont(QFont("Segoe UI", 9)); lbl_sub.setProperty("dim", True)
        ...
        _lbl3 = QLabel("Options"); _lbl3.setFont(QFont("Segoe UI", 11, QFont.Weight.Bold))
        lay.addWidget(_lbl3)
        self._host_match_cb = QCheckBox("Match downloads by hostname")
        self._host_match_cb.setChecked(True); self._host_match_cb.setFont(QFont("Segoe UI", 10))
        ...
        hint.setProperty("dim", True); hint.setFont(QFont("Segoe UI", 8))
        hint.setWordWrap(True); lay.addWidget(hint)
        self._bh_sep(lay)

        _lbl4 = QLabel("Actions"); _lbl4.setFont(QFont("Segoe UI", 11, QFont.Weight.Bold))
        lay.addWidget(_lbl4)

        self._preview_btn = QPushButton("🔍  Preview Data")
        self._preview_btn.setProperty("accent", "true"); self._preview_btn.setMinimumHeight(40)
        self._preview_btn.clicked.connect(self._run_preview); lay.addWidget(self._preview_btn)

        self._bh_export_btn = QPushButton("📊  Export to XLSX")
        self._bh_export_btn.setProperty("success", "true"); self._bh_export_btn.setMinimumHeight(40)
        self._bh_export_btn.setEnabled(False)
        self._bh_export_btn.clicked.connect(self._run_export); lay.addWidget(self._bh_export_btn)
        lay.addStretch()

        self._bh_progress = QProgressBar(); self._bh_progress.setValue(0)
        self._bh_progress.setTextVisible(False); self._bh_progress.setFixedHeight(6)
        lay.addWidget(self._bh_progress)

        self._bh_status_lbl = QLabel("Select a History file to begin")
        self._bh_status_lbl.setProperty("dim", True); self._bh_status_lbl.setFont(QFont("Segoe UI", 9))
        self._bh_status_lbl.setWordWrap(True); lay.addWidget(self._bh_status_lbl)
        return side

    def _bh_sep(self, layout):
        f = QFrame(); f.setFrameShape(QFrame.Shape.HLine)
        f.setProperty("sep_line", True); layout.addWidget(f)

    # ─── Main panel ────────────────────────────────────────────────
    def _build_main(self) -> QWidget:
        w   = QWidget(); lay = QVBoxLayout(w)
        lay.setContentsMargins(0, 0, 0, 0); lay.setSpacing(0)
        self._bh_tabs = QTabWidget(); self._bh_tabs.setDocumentMode(True)
        lay.addWidget(self._bh_tabs)
        self._bh_tabs.addTab(self._build_history_tab(),   "📋  Browsing History")
        self._bh_tabs.addTab(self._build_downloads_tab(), "⬇️  Downloads")
        return w

    def _build_history_tab(self) -> QWidget:
        w   = QWidget(); lay = QVBoxLayout(w)
        lay.setContentsMargins(0, 0, 0, 0); lay.setSpacing(0)
        bar = QFrame(); bar.setProperty("hdr", True)
        b_lay = QHBoxLayout(bar); b_lay.setContentsMargins(10, 7, 10, 7); b_lay.setSpacing(8)
        self._hist_count_lbl = QLabel("No data loaded  —  click Preview Data")
        self._hist_count_lbl.setProperty("dim", True); self._hist_count_lbl.setFont(QFont("Segoe UI", 9))
        b_lay.addWidget(self._hist_count_lbl); b_lay.addStretch()
        lbl_f = QLabel("Filter:"); lbl_f.setFont(QFont("Segoe UI", 9))
        lbl_f.setProperty("dim", True); b_lay.addWidget(lbl_f)
        self._hist_filter = QLineEdit(); self._hist_filter.setPlaceholderText("Search URL or title…")
        self._hist_filter.setFixedWidth(220)
        self._hist_filter.textChanged.connect(self._filter_history); b_lay.addWidget(self._hist_filter)
        self._dl_only_btn = QPushButton("⬇️ Downloads Only"); self._dl_only_btn.setCheckable(True)
        ...
        b_lay.addWidget(self._dl_only_btn); lay.addWidget(bar)
        self._hist_table = self._make_table(
            ["UTC Time", "URL", "Title", "Downloaded", "Downloaded Files"],
            [185, 360, 220, 100, 300]); lay.addWidget(self._hist_table)
        return w
    ...
    def _build_downloads_tab(self) -> QWidget:
        ...
        self._dl_count_lbl.setProperty("dim", True); self._dl_count_lbl.setFont(QFont("Segoe UI", 9))
        b_lay.addWidget(self._dl_count_lbl); b_lay.addStretch()
        lbl_f2 = QLabel("Filter:"); lbl_f2.setFont(QFont("Segoe UI", 9))
        lbl_f2.setProperty("dim", True); b_lay.addWidget(lbl_f2)
        self._dl_filter = QLineEdit(); self._dl_filter.setPlaceholderText("Search file name or URL…")
        self._dl_filter.setFixedWidth(220)
        self._dl_filter.textChanged.connect(self._filter_downloads); b_lay.addWidget(self._dl_filter)
        lay.addWidget(bar)
        self._dl_table = self._make_table(
            ["ID", "File Name", "Start Time (UTC)", "End Time (UTC)", "State", "Size", "Source URL", "Final URL"],
            [45, 200, 190, 190, 100, 80, 300, 300]); lay.addWidget(self._dl_table)
        return w
    ...
    def _make_table(self, headers: List[str], widths: List[int]) -> QTableWidget:
        ...
        t.verticalHeader().setDefaultSectionSize(26); t.verticalHeader().setVisible(False)
        t.setColumnCount(len(headers)); t.setHorizontalHeaderLabels(headers); t.setShowGrid(True)
        for i, w in enumerate(widths): t.setColumnWidth(i, w)
        return t

    # ─── Footer ───────────────────────────────────────────────────
    def _build_footer(self) -> QFrame:
        f = QFrame(); f.setProperty("hdr", True); f.setMaximumHeight(30)
        lay = QHBoxLayout(f); lay.setContentsMargins(14, 4, 14, 4)
        lbl = QLabel("·")
        lbl.setFont(QFont("Segoe UI", 9)); lbl.setProperty("dim", True)
        lay.addWidget(lbl); lay.addStretch()
        return f

    # ─── File / dir selection ─────────────────────────────────────
    def _select_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Select Edge / Chrome History file(s)", "", "All files (*.*)")
        if files:
            self._history_files = list(files); n = len(files)
            self._files_lbl.setText(os.path.basename(files[0]) if n == 1 else f"{n} files selected")
            self._bh_status_lbl.setText(f"{n} file(s) selected — click 🔍 Preview Data")
            self._loaded_data = None; self._bh_export_btn.setEnabled(False)
            self._hist_table.setRowCount(0); self._dl_table.setRowCount(0)
            self._hist_count_lbl.setText("No data loaded  —  click Preview Data")
            self._dl_count_lbl.setText("No data loaded")

    def _browse_outdir(self):
        path = QFileDialog.getExistingDirectory(self, "Select output folder", self._output_dir)
        if path: self._output_dir = path; self._out_dir_edit.setText(path)

    # ─── Preview ──────────────────────────────────────────────────
    def _run_preview(self):
        if not self._history_files:
            QMessageBox.warning(self, "No File Selected", "Please select one or more History files first."); return
        if self._worker and self._worker.isRunning(): return
        db_path  = self._history_files[0]; use_host = self._host_match_cb.isChecked()
        self._set_busy(True)
        self._bh_status_lbl.setText(f"Loading  {os.path.basename(db_path)} …")
        self._bh_progress.setRange(0, 0)
        self._worker = _BHLoadWorker(db_path, use_host)
        self._worker.finished.connect(self._on_load_done)
        self._worker.error.connect(self._on_load_error)
        self._worker.start()

    def _on_load_done(self, data: Dict):
        self._loaded_data = data; self._all_hist_rows = data["history"]; self._all_dl_rows = data["downloads"]
        self._bh_progress.setRange(0, 100); self._bh_progress.setValue(100)
        self._set_busy(False); self._bh_export_btn.setEnabled(True)
        hist_n = data["hist_count"]; dl_n = data["dl_count"]
        dl_hits = sum(1 for h in data["history"] if h["downloaded"])
        trunc_h = f"  (showing first {_BH_PREVIEW_LIMIT:,})" if hist_n > _BH_PREVIEW_LIMIT else ""
        trunc_d = f"  (showing first {_BH_PREVIEW_LIMIT:,})" if dl_n   > _BH_PREVIEW_LIMIT else ""
        self._hist_count_lbl.setText(f"{hist_n:,} history records{trunc_h}  ·  {dl_hits:,} correlated with downloads")
        self._dl_count_lbl.setText(f"{dl_n:,} download records{trunc_d}")
        self._bh_status_lbl.setText(f"Loaded  ·  {hist_n:,} history  ·  {dl_n:,} downloads  ·  {dl_hits:,} correlated")
        self._render_history_table(data["history"][:_BH_PREVIEW_LIMIT])
        self._render_downloads_table(data["downloads"][:_BH_PREVIEW_LIMIT])
        if len(self._history_files) > 1:
            self._bh_status_lbl.setText(self._bh_status_lbl.text() + f"  ·  Previewing 1/{len(self._history_files)} files")
        QTimer.singleShot(3000, lambda: self._bh_progress.setValue(0))

    def _on_load_error(self, err: str):
        self._bh_progress.setRange(0, 100); self._bh_progress.setValue(0)
        self._set_busy(False); self._bh_status_lbl.setText(f"Error: {err[:80]}")
        QMessageBox.critical(self, "Load Error", f"Failed to load history database:\n\n{err}\n\nSee log:\n{_BH_ERROR_LOG}")

    def _set_busy(self, busy: bool):
        self._preview_btn.setEnabled(not busy)
        self._bh_export_btn.setEnabled(not busy and self._loaded_data is not None)

    # ─── Table rendering ──────────────────────────────────────────
    def _render_history_table(self, rows: List[Dict]):
        t     = THEME["dark" if self._dark else "light"]
        green = QColor(t["green"]); dim = QColor(t["fg3"])
        self._hist_table.setUpdatesEnabled(False); self._hist_table.setRowCount(len(rows))
        for i, row in enumerate(rows):
            items = [row["utc_time"], row["url"], row["title"],
                     "✅  Yes" if row["downloaded"] else "—", row["downloaded_files"]]
    ...
    def _render_downloads_table(self, rows: List[Dict]):
        self._dl_table.setUpdatesEnabled(False); self._dl_table.setRowCount(len(rows))
        t   = THEME["dark" if self._dark else "light"]
        grn = QColor(t["green"]); org = QColor(t["orange"]); red = QColor(t["red"]); dim = QColor(t["fg3"])
        for i, row in enumerate(rows):
            vals = [str(row.get("id","")), row.get("file_name",""), row.get("start_utc",""),
                    row.get("end_utc",""), row.get("state",""), row.get("size",""),
                    row.get("source_url",""), row.get("final_url","")]
            for j, val in enumerate(vals):
                item = QTableWidgetItem(str(val))
                if j == 4:
                    if val == "Complete":   item.setForeground(grn)
                    elif val == "In Progress": item.setForeground(org)
                    elif val in ("Cancelled","Interrupted"): item.setForeground(red)
                    item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                if j == 5: item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
                self._dl_table.setItem(i, j, item)
        self._dl_table.setUpdatesEnabled(True)

    # ─── Filtering ────────────────────────────────────────────────
    def _filter_history(self, text: str):
        if not self._all_hist_rows: return
        dl_only = self._dl_only_btn.isChecked(); needle = text.lower()
        rows = [r for r in self._all_hist_rows
                if (not needle or needle in r["url"].lower() or needle in r["title"].lower())
                and (not dl_only or r["downloaded"])]
        self._render_history_table(rows[:_BH_PREVIEW_LIMIT])
        dl_hits = sum(1 for r in rows if r["downloaded"])
        self._hist_count_lbl.setText(f"Showing {len(rows):,} of {len(self._all_hist_rows):,}  ·  {dl_hits:,} with downloads")

    def _filter_downloads(self, text: str):
        if not self._all_dl_rows: return
        needle = text.lower()
        rows = [r for r in self._all_dl_rows
                if not needle or needle in r.get("file_name","").lower()
                or needle in r.get("source_url","").lower() or needle in r.get("final_url","").lower()]
        self._render_downloads_table(rows[:_BH_PREVIEW_LIMIT])
        self._dl_count_lbl.setText(f"Showing {len(rows):,} of {len(self._all_dl_rows):,} download records")

    # ─── Export ───────────────────────────────────────────────────
    def _run_export(self):
        if not self._loaded_data:
            QMessageBox.warning(self, "No Data", "Run Preview first to load data."); return
        if not HAS_OPENPYXL:
            QMessageBox.critical(self, "Missing Dependency", "openpyxl is required.\n\nInstall with:\n  pip install openpyxl"); return
        ...
        try:
            out_path, hist_n, dl_n = _bh_export_xlsx(self._loaded_data, outdir, prefix)
            msg = f"Saved:\n{out_path}\n\nHistory rows : {hist_n:,}\nDownloads rows: {dl_n:,}"
            if len(self._history_files) > 1:
                extra = []
                for extra_path in self._history_files[1:]:
                    try:
                        ed = _bh_load_data(extra_path, self._host_match_cb.isChecked())
                        el = os.path.basename(os.path.dirname(extra_path)).replace(" ","_") or prefix
                        ep, h, d = _bh_export_xlsx(ed, outdir, el)
                        extra.append(f"  {ep}  (H:{h:,} D:{d:,})")
                    except Exception as e:
                        extra.append(f"  ERROR {os.path.basename(extra_path)}: {e}")
                if extra: msg += "\n\nAdditional files:\n" + "\n".join(extra)
            self._bh_status_lbl.setText(f"Exported — {hist_n:,} history + {dl_n:,} downloads")
    ...
    def update_dark_mode(self, dark: bool):
        """Called by OSINTApp when user toggles theme."""
        self._dark = dark
        if self._loaded_data:
            self._render_history_table(self._all_hist_rows[:_BH_PREVIEW_LIMIT])
            self._render_downloads_table(self._all_dl_rows[:_BH_PREVIEW_LIMIT])
...
class OSINTApp(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("MultiOSINT · OSINT Multi-Search")
        try:
    ...
    def _apply_theme(self):
        ...
        # Propagate theme to Browser History panel
        if hasattr(self, "_bh_panel"):
            self._bh_panel.update_dark_mode(self._dark)
    ...
    def _build_ui(self):
        ...
        osint_body = QSplitter(Qt.Orientation.Horizontal)
        osint_body.setHandleWidth(1)
        osint_body.addWidget(self._build_sidebar())
        osint_body.addWidget(self._build_main())
        osint_body.setSizes([310, 900])
        self._body_stack.addWidget(osint_body)

        self._bh_panel = BrowserHistoryPanel(dark=self._dark)
        self._body_stack.addWidget(self._bh_panel)
    ...
    def _build_header(self):
        ...
        for label, slot in [("🔍 OSINT Lookup", self._mode_osint),
                             ...
                             ("🌐 Browser History", self._mode_browser_history)]:
    ...
    def _mode_osint(self):
        self._mode = "osint"
        for i, btn in enumerate(self._mode_btns): btn.setChecked(i == 0)
        if hasattr(self, "_body_stack"):   self._body_stack.setCurrentIndex(0)
        if hasattr(self, "_panel_stack"):  self._panel_stack.setCurrentIndex(0)

    def _mode_phishing(self):
        self._mode = "phishing"
        for i, btn in enumerate(self._mode_btns): btn.setChecked(i == 1)
        if hasattr(self, "_body_stack"):   self._body_stack.setCurrentIndex(0)
        if hasattr(self, "_panel_stack"):  self._panel_stack.setCurrentIndex(1)

    def _mode_browser_history(self):
        self._mode = "browser_history"
        for i, btn in enumerate(self._mode_btns): btn.setChecked(i == 2)
        if hasattr(self, "_body_stack"):   self._body_stack.setCurrentIndex(1)
    ...
    def _build_results_tab(self):
        ...
        self._results_te = _PlainCopyTextEdit(); self._results_te.setReadOnly(True)
    ...
    def _build_ai_tab(self):
        ...
        self._ai_te = _PlainCopyTextEdit(); self._ai_te.setReadOnly(True)
    ...
    def _build_urls_tab(self):
        ...
        lay.addWidget(lbl)
        self._urls_te = _PlainCopyTextEdit(); self._urls_te.setReadOnly(True)
        ...
        def worker():
            for i, ioc in enumerate(iocs):
                try:
                    ...
                    r    = {"_ioc": ioc, "_ioc_type": "?", "_error": str(exc)}
                    text = f"\n{'='*62}\n  IOC: {ioc}  [ERROR]\n{'='*62}\n  ⚠️  {exc}\n"
    ...
    def _lookup_single(self, cleaned):
        ...
        if ioc_type == "email":
            ...
            futures["er"]   = ex.submit(self.osint.check_emailrep,  cleaned)
            futures["lc"]   = ex.submit(self.osint.check_leakcheck, cleaned)
        else:
        ...
        if ioc_type == "ip":
            ...
            futures["tf"]   = ex.submit(self.osint.check_threatfox,      cleaned)
            futures["uh"]   = ex.submit(self.osint.check_urlhaus,        cleaned, "url")
            futures["pt"]   = ex.submit(self.osint.check_phishtank,      cleaned)
            futures["ipqs"] = ex.submit(self.osint.check_ipqs,           cleaned, "url")
            futures["us"]   = ex.submit(self.osint.check_urlscan_search, cleaned, "url")
        results = {}
    ...
    def _format_result(self, results):
        ...
        if ioc_type == "email":
            ...
            if bvip and not bvip.get("bvip_error") and bvip.get("bvip_count",0) > 0:
                ...
                for item in bvip.get("bvip_results",[])[:20]:
                    ...
                    if cats: ln += f"  [{cats}]"
                    lines.append(ln)
            if er and not er.get("er_error"):
                lines += ["\n▸ EmailRep.io  (free)", f"  {er['er_verdict']}"]
                if er.get("er_credentials_leaked"): lines.append("  ⚠️  Credentials leaked in past breach")
                if er.get("er_data_breach"):        lines.append("  ⚠️  Associated with known data breach")
                if er.get("er_profiles"):           lines.append(f"  Profiles: {', '.join(er['er_profiles'][:6])}")
            if lc and not lc.get("lc_error"):
                lines += ["\n▸ LeakCheck.io  (free)", f"  {lc['lc_verdict']}"]
                for src in lc.get("lc_sources",[])[:10]:
        ...
        if ioc_type == "ip":
            ...
            if us and not us.get("us_error"):
                lines += ["\n▸ URLScan.io  (search — free)",
                          f"  {us.get('us_verdict','')}  ·  Malicious hits: {us.get('us_malicious',0)}  Benign: {us.get('us_benign',0)}"]
                if us.get("us_tags"): lines.append(f"  Tags: {us['us_tags']}")
            if pt and not pt.get("pt_error"):
                lines += ["\n▸ PhishTank  (free)", f"  {pt.get('pt_verdict','')}"]
            if ipqs and not ipqs.get("ipqs_error") and ipqs.get("ipqs_fraud_score") not in (None,"","0",0):
                lines += ["\n▸ IPQualityScore  (URL)",
                          f"  {ipqs.get('ipqs_verdict','')}  FraudScore: {ipqs.get('ipqs_fraud_score','')}"]
    ...
    def _ph_render_all(self):
        ...
        self._status_lbl.setText(f"Analysis complete — {lv}  Score: {v['score']}/200")
